...
class Constants(object):
    # MIDI note range is the main constant. Everything is based on it.
    # I.e. piano keys range is 21-108 (88 keys from A0 to C8)
    MIDI_NOTE_RANGE = range(0, 128)

    # Internal note notation
    NOTES = "C CD D DE E F FG G GA A AB B".split()
    BLACK_KEYS = "CD DE FG GA AB".split()

    # Notes are in integer notation (0..11)
...
class Note(object):
    ...
    def is_white_key(self):
        ...
        return not (Constants.NOTES_DIC[self._midi_note_number % 12]
                    in Constants.BLACK_KEYS)
    ...
    def _str_note_to_number(self, value):
        ...
        return (12 * (_input_octave + 1) +
                Constants.NOTES_NUM_DIC[_input_note])
